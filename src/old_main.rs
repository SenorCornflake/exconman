...
// Check if the file of the setting exists
fn validate_setting(setting: &Setting, registry_path: &str) -> Result<(), ()> {
    let result = std::fs::metadata(util::expand_home(&setting.file));
...
fn set(setting_name: String, value: String, stop_at_first_match: bool, registry: &Vec<Setting>) {
    ...
    let file = std::fs::read_to_string(&util::expand_home(&setting.file)).unwrap();
    ...
    if setting.value_is_file {
        let result = std::fs::read_to_string(util::expand_home(&value));
    ...
    match std::fs::write(util::expand_home(&setting.file), modified_file) { _ => {}}
}
...
// Get registry, whether file or directory
...
fn main() {
    ...
    let registry_path: Result<String, ()> = if registry_path.is_some() {
        ...
        let registry_file: String = util::expand_home(registry_file);

        let registry_dir = "~/.config/exconman/registry";
        let registry_dir: String = util::expand_home(registry_dir);
